...
export function ClientHistory({ data, onChange }: Props) {
    ...
    <div className="space-y-6 pt-4">
      ...
      {data.assessmentReason === '90day' && (
        <div className="bg-blue-50 rounded-xl p-4 space-y-3">
          <p className="text-sm font-medium text-blue-800">Re-assessment visit due to:</p>
      ...
      <TextInput
        label="Desired services how many times a week?"
        value={data.servicesPerWeek}
        onChange={e => onChange({ servicesPerWeek: e.target.value })}
      />
      <div className="flex gap-4">
        <label className="flex items-center gap-2 cursor-pointer min-h-[44px] px-3 py-2 rounded-lg border-2 transition-all text-sm flex-1
        ...
        <label className="flex items-center gap-2 cursor-pointer min-h-[44px] px-3 py-2 rounded-lg border-2 transition-all text-sm flex-1
          border-gray-200 bg-white hover:border-gray-300">
          <input type="checkbox" checked={data.liveIn} onChange={e => onChange({ liveIn: e.target.checked })} className="accent-amber-600 w-4 h-4" />
          Live-in
        </label>
      </div>

      <TextInput label="When does the client want services to begin?" type="date" value={data.serviceStartDate} onChange={e => onChange({ serviceStartDate: e.target.value })} />

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Preferred Service Times</label>
        {data.preferredTimes.map((time, i) => (
          <div key={i} className="flex items-center gap-2">
            <TextInput label="" type="time" value={time.from} onChange={e => updateTime(i, 'from', e.target.value)} className="flex-1" />
            <span className="text-gray-400 text-sm mt-1">to</span>
            <TextInput label="" type="time" value={time.to} onChange={e => updateTime(i, 'to', e.target.value)} className="flex-1" />
          </div>
        ))}
      </div>
      ...
      {/* Conditional fields based on health history */}
      {data.healthHistory.includes('Recent fall') || true ? (
      ...
      {data.otherProviders.map((provider, index) => (
        <div key={index} className="bg-gray-50 rounded-xl p-4 space-y-3">
          <div className="flex justify-between items-center">
            <p className="text-sm font-medium text-gray-500">Provider {index + 1}</p>
            {data.otherProviders.length > 1 && (
              <button type="button" onClick={() => removeProvider(index)} className="text-xs text-red-500 hover:text-red-700">Remove</button>
            )}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <TextInput label="Phone" type="tel" value={provider.phone} onChange={e => updateProvider(index, 'phone', e.target.value)} />
      ...
      <button type="button" onClick={addProvider} className="text-sm text-amber-600 hover:text-amber-700 font-medium">
